...
def update_product_field(product_id, field_name, new_value):
    conn = get_db_connection()
    products_table = ['name_product', 'photo', 'proce', 'size']
    products_details_table = ['category', 'info_product', 'url']
    ...
    try:
        if field_name in products_table:
            ...
            raise ValueError(f'Unvailable row: {field_name}')
...
async def start_sending_products(message: types.Message):
    ...
    button = InlineKeyboardButton('Look', callback_data='show_all_update')
    keyboard.add(button)

    await message.answer('if ypu push, all products will quit!', reply_markup=keyboard)
...
async def send_all_products(callback_query: types.CallbackQuery):
    ...
    if products:
        for product in products:
            keyboard = InlineKeyboardMarkup(resize_keyboard=True)
            button = InlineKeyboardButton('Edit', callback_data=f"edit{product['product_id']}")
            keyboard.add(button)
...
def register_update_products_handler(dp: Dispatcher):
    dp.register_message_handler(start_sending_products, commands=['product_update'])
    ...
    dp.register_message_handler(set_new_photo, state=EditProduct.for_photo)